import type { Category } from '@/domain';

/**
 * The launch categories. Shaped exactly like the future Firestore
 * `categories` documents so this array can be replaced by a network fetch in a
 * later slice with no consumer changes.
 */
export const CATEGORIES = [
  {
    id: 'events',
    name: 'Historic Events',
    icon: 'flag',
    colour: '#E7B84C',
    description: 'Turning points that reshaped the world.',
    difficulty: 'easy',
    active: true,
    premiumOnly: false,
    displayOrder: 0,
  },
  {
    id: 'battles',
    name: 'Battles',
    icon: 'swords',
    colour: '#E8564E',
    description: 'Clashes that decided the fate of empires.',
    difficulty: 'medium',
    active: true,
    premiumOnly: false,
    displayOrder: 1,
  },
  {
    id: 'people',
    name: 'Famous People',
    icon: 'person',
    colour: '#57BE8F',
    description: 'When history’s icons were born.',
    difficulty: 'medium',
    active: true,
    premiumOnly: false,
    displayOrder: 2,
  },
  {
    id: 'technology',
    name: 'Technology',
    icon: 'cpu',
    colour: '#A9B6C2',
    description: 'Inventions that changed how we live.',
    difficulty: 'medium',
    active: true,
    premiumOnly: false,
    displayOrder: 3,
  },
  {
    id: 'exploration',
    name: 'Exploration & Discovery',
    icon: 'compass',
    colour: '#3EB8C9',
    description: 'Voyages and firsts that redrew the map.',
    difficulty: 'medium',
    active: true,
    premiumOnly: false,
    displayOrder: 4,
  },
  {
    id: 'treaties',
    name: 'Treaties',
    icon: 'handshake',
    colour: '#D9A066',
    description: 'The signatures that ended wars and drew borders.',
    difficulty: 'hard',
    active: true,
    premiumOnly: false,
    displayOrder: 5,
  },
  {
    id: 'sport',
    name: 'Sport',
    icon: 'trophy',
    colour: '#F08A4B',
    description: 'First finals, record runs and famous upsets.',
    difficulty: 'medium',
    active: true,
    premiumOnly: false,
    displayOrder: 6,
  },
  {
    id: 'trade',
    name: 'Money & Trade',
    icon: 'coins',
    colour: '#C9A227',
    description: 'Coins, companies, canals and crashes.',
    difficulty: 'hard',
    active: true,
    premiumOnly: false,
    displayOrder: 7,
  },
  // Premium categories sit at the end so the free list reads uninterrupted.
  {
    id: 'arts',
    name: 'Arts & Culture',
    icon: 'palette',
    colour: '#B07BD9',
    description: 'Masterpieces, premieres and cultural icons.',
    difficulty: 'medium',
    active: true,
    premiumOnly: true,
    displayOrder: 8,
  },
  {
    id: 'philosophy',
    name: 'Philosophy',
    icon: 'owl',
    colour: '#4FA3A5',
    description: 'The thinkers and ideas that questioned everything.',
    difficulty: 'hard',
    active: true,
    premiumOnly: true,
    displayOrder: 9,
  },
  {
    id: 'regional',
    name: 'Regional',
    icon: 'globe',
    colour: '#4C8FE7',
    description: 'Pick a region and play its defining moments.',
    difficulty: 'medium',
    active: true,
    premiumOnly: true,
    displayOrder: 10,
  },
  {
    id: 'space',
    name: 'Space',
    icon: 'rocket',
    colour: '#6C7BE8',
    description: 'Launches, landings and the race beyond Earth.',
    difficulty: 'medium',
    active: true,
    premiumOnly: true,
    displayOrder: 11,
  },
] satisfies readonly Category[];
